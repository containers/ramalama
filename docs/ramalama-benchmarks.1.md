...
Results are stored in a local SQLite database. The database location is
shown in `ramalama benchmarks --help` and can be overridden via
`ramalama.benchmarks.db_path` in `ramalama.conf`.
