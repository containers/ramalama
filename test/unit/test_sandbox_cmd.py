...
def test_sandbox_internal_network_creation_failure_raises(monkeypatch):
...
def test_sandbox_internal_network_rejects_discovered_model(monkeypatch):
    """--internal-network must be rejected when localhost discovery finds a model.

    Without this check the code would skip model-server startup and return
    before creating the internal network, silently ignoring --internal-network.
    """
    ...
    with pytest.raises(ValueError, match="--internal-network requires ramalama to start its own model server"):
