...
def test_list_benchmarks_orders_and_limits(tmp_path, monkeypatch):
    ...
    cfg_b = llama_bench.TestConfiguration(
        container_image="image-b",
        container_runtime="podman",
        inference_engine="engine-b",
        runtime_args={"threads": 2},
    )

    cfg_a_id = db.save_container_configuration(cfg_a)
    cfg_b_id = db.save_container_configuration(cfg_b)

    res_a = llama_bench.LlamaBenchResult(
        build_commit="a",
        build_number=1,
        model_filename="model-a.gguf",
        n_threads=1,
        n_prompt=8,
        n_gen=16,
        avg_ts=1.0,
    ...
    res_b = llama_bench.LlamaBenchResult(
        build_commit="b",
        build_number=2,
        model_filename="model-b.gguf",
        n_threads=2,
        n_prompt=8,
        n_gen=16,
        avg_ts=2.0,
    ...
    db.save_llama_bench_results(cfg_a_id, [res_a])
    db.save_llama_bench_results(cfg_b_id, [res_b])

    con = sqlite3.connect(db_path)
    try:
        cur = con.cursor()
        cur.execute("SELECT id FROM llama_bench ORDER BY id")
        ids = [row[0] for row in cur.fetchall()]
        assert len(ids) == 2
        cur.execute(
            "UPDATE llama_bench SET created_at = ? WHERE id = ?",
            ("2024-01-01 00:00:00", ids[0]),
        )
        cur.execute(
            "UPDATE llama_bench SET created_at = ? WHERE id = ?",
            ("2024-01-02 00:00:00", ids[1]),
        )
        con.commit()
    finally:
        con.close()

    all_rows = list(db.list_benchmarks())
    limited_rows = list(db.list_benchmarks(limit=1))
    offset_rows = list(db.list_benchmarks(limit=1, offset=1))

    assert len(all_rows) == 2
    assert all_rows[0].engine == "engine-b"
    assert all_rows[0].result.avg_ts == 2.0
    assert all_rows[0].created_at == "2024-01-02 00:00:00"
    assert all_rows[1].engine == "engine-a"
    assert all_rows[1].result.avg_ts == 1.0
    assert all_rows[1].created_at == "2024-01-01 00:00:00"

    assert len(limited_rows) == 1
    assert limited_rows[0].engine == "engine-b"

    assert len(offset_rows) == 1
    assert offset_rows[0].engine == "engine-a"
