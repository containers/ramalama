...
def test_list_models_timezones_in_utc(monkeypatch):
    ...
    manifest_output = (
        '{"name":"oci://localhost/manifest:latest","modified":"2026-01-01 00:00:00 +0000","size":234,"ID":"sha256:b"},'
    )
    artifact_output = (
        '{"name":"oci://localhost/artifact:latest","created":"2026-01-01 00:00:00 +0000","size":"1KB","ID":"sha256:c"},'
    )
    ...
    def fake_run_cmd(args, **kwargs):
        ...
        if args[:4] == ["podman", "images", "--filter", "manifest=true"]:
            return _result(manifest_output)
        if args[:3] == ["podman", "artifact", "ls"]:
            return _result(artifact_output)
        if args[:3] == ["podman", "artifact", "inspect"]:
            return _result(json.dumps(artifact_inspect_output))
        raise AssertionError(f"Unexpected command: {args}")
