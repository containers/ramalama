...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "model.safetensors",
            "main",
            "huggingface.co/LiheYoung/depth-anything-small-hf",
        ),
        (
...
def test_rag_args_clears_port_override():
    ...
    assert rag_args.rag_stack_generated is False
...
def test_rag_args_generates_shared_stack_name():
    ...
    assert rag_args.rag_stack_generated is True
    assert args.name == f"ramalama-model-{unique}"


def test_rag_args_explicit_genname_lookalike_keeps_user_form():
    """A user --name that looks like genname() still uses {name}-model."""
    from ramalama.cli import _rag_args
    ...
    assert rag_args.name == "ramalama-abcdefghij"
    assert rag_args.rag_stack_generated is False
    assert args.name == "ramalama-abcdefghij-model"
