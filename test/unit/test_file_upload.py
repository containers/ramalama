...
from ramalama.file_upload.file_types.pdf import PDFFileUpload
...
class TestPDFFileUpload:
    """Test the PDF file upload handler."""

    def test_pdf_file_upload_initialization(self):
        """Test that PDFFileUpload can be initialized."""
        file_path = "/path/to/test.pdf"
        uploader = PDFFileUpload(file_path)
        assert uploader.file == file_path
        assert isinstance(uploader, BaseFileUpload)

    def test_pdf_file_upload_load_content(self):
        """Test loading content from a PDF file (currently returns empty string)."""
        uploader = PDFFileUpload("/path/to/test.pdf")
        result = uploader.load()
...
class TestSupportedExtensions:
    ...
    def test_supported_extensions_mapping(self):
        ...
        assert SUPPORTED_EXTENSIONS['.pdf'] == PDFFileUpload
...
class TestFileUploadIntegration:
    ...
    def test_file_upload_with_pdf_files(self):
        """Test uploading PDF files (currently returns empty string)."""
        with tempfile.TemporaryDirectory() as tmp_dir:
            pdf_file = os.path.join(tmp_dir, "test.pdf")
            with open(pdf_file, "w") as f:
                f.write("PDF content (simulated)")
