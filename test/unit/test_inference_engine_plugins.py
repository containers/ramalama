...
class TestLlamaCppPlugin:
    ...
    @patch("ramalama.plugins.runtimes.inference.llama_cpp.BenchmarksManager")
    @patch("ramalama.plugins.runtimes.inference.llama_cpp.print_bench_results")
    @patch("ramalama.plugins.runtimes.inference.llama_cpp.parse_json", return_value=[])
    @patch("ramalama.plugins.runtimes.inference.llama_cpp.set_accel_env_vars")
    @patch("ramalama.plugins.runtimes.inference.llama_cpp.assemble_command", return_value=["llama-bench"])
    @patch("ramalama.plugins.runtimes.inference.llama_cpp.accel_image", return_value="quay.io/ramalama/rocm:latest")
    @patch("ramalama.plugins.runtimes.inference.llama_cpp.ensure_image", side_effect=lambda _e, image, **_k: image)
    @patch("ramalama.plugins.runtimes.inference.llama_cpp.ActiveConfig")
    @patch("ramalama.plugins.runtimes.inference.llama_cpp.New")
    def test_bench_recomputes_image_from_backend(
        self,
        mock_new,
        mock_active_config,
        mock_ensure_image,
        ...
        # Regression for containers/ramalama#2823: `ramalama bench --backend X` must honor the
        # backend like run/serve do. The eager --image default is computed before --backend is
        # synced into the config, so _bench_handler must re-derive args.image via accel_image.
        config = MagicMock()
        config.pull = "missing"
        config.engine = "podman"
        config.benchmarks.disable = True
        mock_active_config.return_value = config

        model = MagicMock()
        model.engine.run_process.return_value.stdout = "[]"
        mock_new.return_value = model

        ns = make_ns(container=True, MODEL="ollama://mymodel")
        ns.image = "quay.io/ramalama/ramalama:latest"  # stale default (auto/vulkan), pre-backend-sync
        ns.engine = "podman"
        ns.runtime = "llama.cpp"
        ns.format = "table"

        self.plugin._bench_handler(ns)

        # The image was re-derived from the (backend-synced) config, not left at the stale default.
        mock_accel_image.assert_called_once_with(config)
        assert ns.image == "quay.io/ramalama/rocm:latest"

    @patch("ramalama.plugins.runtimes.inference.llama_cpp.ensure_image")
    @patch("ramalama.plugins.runtimes.inference.llama_cpp.set_accel_env_vars")
    @patch("ramalama.plugins.runtimes.inference.llama_cpp.assemble_command", return_value=["llama-bench"])
    @patch("ramalama.plugins.runtimes.inference.llama_cpp.New")
    def test_bench_dryrun_does_not_recompute_image(
        self, mock_new, _assemble, _set_env, mock_ensure_image, container_image_is_ggml
    ):
        # In dryrun mode bench keeps the resolved default image, matching run/serve behavior.
        model = make_transport_model()
        mock_new.return_value = model

        ns = make_ns(container=True, dryrun=True, MODEL="ollama://mymodel")
        ns.image = "quay.io/ramalama/ramalama:latest"
        ns.engine = "podman"
        ns.runtime = "llama.cpp"
        ns.subcommand = "bench"
        ns.format = "table"
        ...
        assert ns.image == "quay.io/ramalama/ramalama:latest"
