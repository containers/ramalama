...
@test "ramalama pull ollama cache" {
    ...
    random_image_name=i_$(safename)
    run_ramalama 1 pull ${random_image_name}
    is "$output" "Error: Manifest for ${random_image_name} was not found in the Ollama registry"
...
@test "ramalama URL" {
      ...
      for url in $file_url $https_url; do
          ...
          run_ramalama list
          expected_url=$(sed "s/file\:\//url\:/" <<< $url)
...
@test "ramalama file URL" {
      ...
      rm ${model}
      run_ramalama list
      is "$output" ".*$expected_url does not exist" "URL exists"
      run_ramalama rm $url
      run_ramalama list
      assert "$output" !~ ".*$expected_url" "URL no longer exists"
}
