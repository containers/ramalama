...
function _prefetch() {
     ...
     run_ramalama '?' image exists "$want"
     if [[ $status -eq 0 ]]; then
         return
     fi
    ...
    local cachepath="${RAMALAMA_IMAGECACHE}/${cachename}.tar"
    if [[ ! -e "$cachepath" ]]; then
    ...
    # with skopeo, not ramalama, in order to preserve metadata
...
function basic_teardown() {
    ...
    # and RAMALAMA_BATS_LEAK_CHECK is set.
    # As these ramalama commands are slow we do not want to do this by default
    # and only provide this as opt in option. (#22909)
    if [[ "$BATS_TEST_COMPLETED" -eq 1 ]] && [ $exit_code -eq 0 ] && [ -n "$RAMALAMA_BATS_LEAK_CHECK" ]; then
        leak_check
        exit_code=$((exit_code + $?))
    fi
...
function archive_image() {
    ...
    run_ramalama save -o $archive $image
}
...
function clean_setup() {
    ...
    for action in "${actions[@]}"; do
        ...
        if [[ $status -ne 0 ]]; then
            echo "# [teardown] $_LOG_PROMPT ramalama $action" >&3
            ...
            if [[ $status -eq 124 ]]; then
                echo "# [teardown] $_LOG_PROMPT ramalama system locks" >&3
                run $RAMALAMA system locks
                for line in "${lines[*]}"; do
                    echo "# $line" >&3
                done
            fi
    ...
    for line in "${lines[@]}"; do
        ...
        if [[ "$1" == "$RAMALAMA_TEST_IMAGE_FQN" ]]; then
            if [[ -z "$RAMALAMA_TEST_IMAGE_ID" ]]; then
                # This will probably only trigger the 2nd time through setup
                RAMALAMA_TEST_IMAGE_ID=$2
            fi
            found_needed_image=1
        else
            # Always remove image that doesn't match by name
            echo "# setup(): removing stray image $1" >&3
            run_ramalama rm --force "$1" >/dev/null 2>&1 || true

            # Tagged image will have same IID as our test image; don't rm it.
            if [[ $2 != "$RAMALAMA_TEST_IMAGE_ID" ]]; then
                echo "# setup(): removing stray image $2" >&3
                run_ramalama rm --force "$2" >/dev/null 2>&1 || true
            fi
        fi
    done
    ...
    if [[ -z "$found_needed_image" ]]; then
        _prefetch $RAMALAMA_TEST_IMAGE_FQN
    fi
}
...
function ramalama_storage_driver() {
    run_ramalama info --format '{{.Store.GraphDriverName}}' >/dev/null
...
# for running an isolated ramalama that will not step on system ramalama. Set:
...
function ramalama_isolation_opts() {
    local path=${1?ramalama_isolation_opts: missing PATH arg}
...
function assert() {
    ...
    if [ -z "$testname" ]; then
        testname="${MOST_RECENT_RAMALAMA_COMMAND:-[no test name given]}"
    fi
...
function is() {
    local actual="$1"
    local expect="$2"
    local testname="${3:-${MOST_RECENT_RAMALAMA_COMMAND:-[no test name given]}}"
...
####################
#
# HEADS UP: Operates on '$lines' array, so, must be invoked after run_ramalama
#
...
############
#
# Set RAMALAMA_TEST_DEBUG to the name of one or more functions you want to debug
#
# Examples:
#
#    $ RAMALAMA_TEST_DEBUG=parse_table bats .
#    $ RAMALAMA_TEST_DEBUG="test_ramalama_images test_ramalama_run" bats .
#
function dprint() {
    test -z "$RAMALAMA_TEST_DEBUG" && return

    caller="${FUNCNAME[1]}"

    # RAMALAMA_TEST_DEBUG is a space-separated list of desired functions
    # e.g. "parse_table test_ramalama_images" (or even just "table")
    for want in $RAMALAMA_TEST_DEBUG; do
...
function find_exec_pid_files() {
    run_ramalama info --format '{{.Store.RunRoot}}'
...
#############################
#
# On some CI systems, 'ramalama run --privileged' emits a useless warning:
#
...
# run 'ramalama help', parse the output looking for 'Available Commands';
# return that list.
