...
from test.conftest import skip_if_docker, skip_if_no_container, skip_if_podman_too_old
...
def test_artifact_with_different_tags():
    ...
    with RamalamaExecWorkspace() as ctx:
        ...
        for tag in tags:
            ctx.check_call(
                ["ramalama", "convert", "--type", "raw", f"file://{test_file}", f"test-tagged-artifact:{tag}"]
            )
