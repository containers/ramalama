...
from test.conftest import skip_if_container, skip_if_docker, skip_if_no_container, skip_if_not_darwin
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            id="check --network is not present by default", marks=skip_if_no_container
        ),
        pytest.param(
            ["--name", "foobar"], r".*--host 0.0.0.0", None, None, True,
            id="check --host default value", marks=skip_if_no_container
        ),
        pytest.param(
        ...
        pytest.param(
            ["--host", "127.1.2.3"],
            r".*--host 127.1.2.3", None, None, True,
            id="check --host"
        ),
        pytest.param(
...
def test_stop_failures():
    with RamalamaExecWorkspace() as ctx:
        ...
        assert re.search(
            r"Error: no container with name or ID \"{}\" found: no such container".format(container_id),
            exc_info.value.output.decode("utf-8"),
        )
