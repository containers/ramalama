...
@pytest.mark.e2e
@skip_if_no_container
def test_router_mode_models_preset_dry_run(shared_ctx, test_model, tmp_path):
    """--models-preset flag adds a bind mount and passes --models-preset to llama-server."""
    presets = tmp_path / "presets.ini"
    presets.write_text("[routing]\n")
    ctx = shared_ctx
    result = ctx.check_output(["ramalama", "-q", "--dryrun", "serve", "--models-preset", str(presets)])
    assert re.search(r".*--mount=type=bind,src=.*,destination=/etc/presets\.ini,ro", result)
    assert re.search(r".*--models-preset /etc/presets\.ini", result)
