...
from test.conftest import skip_if_container, skip_if_darwin, skip_if_docker, skip_if_no_container
...
def test_basic_dry_run():
    ...
    result = check_output(f"ramalama -q --dryrun run {TEST_MODEL} <<< \"Test\"", shell=True)
    assert result.startswith("{} run --rm".format(ramalama_info["Engine"]["Name"]))
    assert not re.search(r".*-t -i", result), "run without terminal"
...
@pytest.mark.e2e
@skip_if_no_container
@skip_if_darwin  # FIXME: Why darwin is excluded here?
@skip_if_docker
