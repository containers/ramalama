    ...
    def _get_llm_summary(
        ...
        try:
            ...
            with urllib.request.urlopen(request, timeout=self.timeout) as response:
                ...
                # Validate summary is useful
                confusion_phrases = [
                    "no story", "just started", "no conversation",
                    "i don't have", "i cannot", "there is no", "cannot summarize",
                ]
                if len(summary) < 20 or any(p in summary.lower() for p in confusion_phrases):
                    logger.warning(f"LLM returned unhelpful summary: {summary[:100]}")
                    return None
