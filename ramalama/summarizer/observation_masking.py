...
class ObservationMasking(ContextStrategy):
    ...
    def __init__(self, max_context_messages: int = 6, max_history: int | None = None):
        ...
        Args:
            ...
            max_history: Alias for max_context_messages (for API compatibility)
        """
        # max_history takes precedence if provided
        effective_max = max_history if max_history is not None else max_context_messages
        super().__init__(effective_max)

    def apply(self, history: list[dict], max_history: int | None = None) -> list[dict]:
        """Apply observation masking to conversation history.

        This is an alias for reduce_context with a simpler interface.

        Args:
            history: List of message dicts
            max_history: Optional override for max messages to retain

        Returns:
            Reduced conversation history with recent messages preserved
        """
        # Use provided max_history or fall back to configured value
        if max_history is not None:
            old_max = self.max_context_messages
            self.max_context_messages = max_history
            result = self.reduce_context(history, n_ctx=8192)  # Default large context
            self.max_context_messages = old_max
            return result
        return self.reduce_context(history, n_ctx=8192)
    ...
    def reduce_context(self, conversation_history: list[dict], n_ctx: int) -> list[dict]:
        ...
        if available_for_old > 50:
            ...
            if key_info:
                ...
                new_history.append({"role": "system", "content": context_note})
