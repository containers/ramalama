...
class Transport(TransportBase):
    ...
    def inspect(
        ...
        json_out = self.get_inspect(show_all, show_all_metadata, get_field, dryrun, as_json=as_json)
        if as_json:
            print(json_out)
        else:
    ...
    def get_inspect(
        ...
        if GGUFInfoParser.is_model_gguf(model_path):
            if not show_all_metadata and get_field == "":
                ...
                return gguf_info.serialize(json=True, all=show_all)

            metadata = GGUFInfoParser.parse_metadata(model_path)
            if show_all_metadata:
                return metadata.serialize(json=True)
        ...
        if SafetensorInfoParser.is_model_safetensor(model_name):
            ...
            return safetensor_info.serialize(json=True, all=show_all)

        return ModelInfoBase(model_name, model_registry, model_path).serialize(json=True)
