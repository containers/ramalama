...
download_binary_and_run_installer() {
    ...
    RECEIPT="${RECEIPT//'\"CARGO_DIST_BINS\"'/\"$_bins_js_array\"}"
    RECEIPT="${RECEIPT//'\"CARGO_DIST_DYLIBS\"'/\"$_libs_js_array\"}"
    RECEIPT="${RECEIPT//'\"CARGO_DIST_STATICLIBS\"'/\"$_staticlibs_js_array\"}"
...
install() {
    ...
    RECEIPT="${RECEIPT//AXO_INSTALL_PREFIX/$_receipt_install_dir}"
    # Also replace the aliases with the arch-specific one
    RECEIPT="${RECEIPT//'"binary_aliases":{}'/\"binary_aliases\":$(json_binary_aliases \"$_arch\")\"}"
    # And replace the install layout
    RECEIPT="${RECEIPT//'"install_layout":"unspecified"'/\"install_layout\":\"$_install_layout\"}"
    if [ "$NO_MODIFY_PATH" = "1" ]; then
        RECEIPT="${RECEIPT//'"modify_path":true'/\"modify_path\":false\"}"
    fi
    ...
    ensure "${_install_dir}/uv" tool install --force --python python3.12 ramalama
